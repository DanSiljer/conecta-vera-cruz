(function () {
  ...
  const board = document.getElementById("bandeiraPuzzleBoard");
  const tray = document.getElementById("bandeiraPuzzleTray");
  if (!board || !tray) return;

  const difficulty = document.getElementById("bandeiraPuzzleDifficulty");
  const startButton = document.getElementById("bandeiraPuzzleStart");
  const showButton = document.getElementById("bandeiraPuzzleShow");
  const resetButton = document.getElementById("bandeiraPuzzleReset");
  const movesOutput = document.getElementById("bandeiraPuzzleMoves");
  const timeOutput = document.getElementById("bandeiraPuzzleTime");
  const placedOutput = document.getElementById("bandeiraPuzzlePlaced");
  const totalOutput = document.getElementById("bandeiraPuzzleTotal");
  const messageOutput = document.getElementById("bandeiraPuzzleMessage");
  const trayTitle = document.getElementById("bandeiraPuzzleTrayTitle");

  const IMAGE_SRC = "assets/img/jogos/bandeira-vera-cruz.png";
  ...
  function createPieceSvg(index, ghost) {
    ...
    const clipId = "bandeiraPieceClip-" + state.size + "-" + index + "-" + Math.random().toString(36).slice(2, 8);
    ...
    svg.classList.add("bandeira-puzzle__piece-svg");
    ...
    outline.classList.add(ghost ? "bandeira-puzzle__ghost-outline" : "bandeira-puzzle__piece-outline");
  ...
  function createPiece(index) {
    ...
    piece.type = "button";
    piece.className = "bandeira-puzzle__piece";
  ...
  function createSlot(index) {
    ...
    const col = index % state.size;
    slot.type = "button";
    slot.className = "bandeira-puzzle__slot";
  ...
  function saveProgress() {
    ...
    results.unshift({
      ...
      activity: "Quebra-cabeça da Bandeira de Vera Cruz · " + difficultyName,
  ...
  function finishGame() {
    ...
    celebrateCompletion();
    messageOutput.innerHTML = "<strong>Quebra-cabeça concluído!</strong> Você montou a Bandeira de Vera Cruz em " + state.moves + " movimentos e " + formatTime(state.seconds) + ". O resultado foi salvo em Meu progresso.";
  ...
  function resetGame() {
    ...
    buildBoard();
    fitPuzzleToScreen();
    tray.innerHTML = '<div class="bandeira-puzzle__tray-empty"><span>🧩</span><strong>As peças aparecerão aqui</strong><p>Escolha o nível e clique em “Começar jogo”.</p></div>';
    updateStats();
  ...
  function showHint() {
    ...
    messageOutput.textContent = "Dica rápida: memorize a posição do brasão, das faixas e das cores da bandeira.";
    window.setTimeout(function () {
  ...
  preload.addEventListener("error", function () {
    resetGame();
    messageOutput.textContent = "A imagem do quebra-cabeça não foi encontrada. Confira assets/img/jogos/bandeira-vera-cruz.png.";
  });
