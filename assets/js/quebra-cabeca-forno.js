(function () {
  ...
  const board = document.getElementById("fornoPuzzleBoard");
  const tray = document.getElementById("fornoPuzzleTray");
  if (!board || !tray) return;

  const difficulty = document.getElementById("fornoPuzzleDifficulty");
  const startButton = document.getElementById("fornoPuzzleStart");
  const showButton = document.getElementById("fornoPuzzleShow");
  const resetButton = document.getElementById("fornoPuzzleReset");
  const movesOutput = document.getElementById("fornoPuzzleMoves");
  const timeOutput = document.getElementById("fornoPuzzleTime");
  const placedOutput = document.getElementById("fornoPuzzlePlaced");
  const totalOutput = document.getElementById("fornoPuzzleTotal");
  ...
  function createPieceSvg(index, ghost) {
    ...
    const clipId = "fornoPieceClip-" + state.size + "-" + index + "-" + Math.random().toString(36).slice(2, 8);
    ...
    svg.classList.add("forno-puzzle__piece-svg");
    ...
    outline.classList.add(ghost ? "forno-puzzle__ghost-outline" : "forno-puzzle__piece-outline");
  ...
  function createPiece(index) {
    ...
    piece.type = "button";
    piece.className = "forno-puzzle__piece";
  ...
  function createSlot(index) {
    ...
    const col = index % state.size;
    slot.type = "button";
    slot.className = "forno-puzzle__slot";
  ...
  function saveProgress() {
    ...
    results.unshift({
      ...
      activity: "Quebra-cabeça do Forno da Penha · " + difficultyName,
  ...
  function finishGame() {
    ...
    celebrateCompletion();
    messageOutput.innerHTML = "<strong>Quebra-cabeça concluído!</strong> Você montou o Forno da Penha em " + state.moves + " movimentos e " + formatTime(state.seconds) + ". O resultado foi salvo em Meu progresso.";
  ...
  function showHint() {
    ...
    messageOutput.textContent = "Dica rápida: memorize a posição do forno, das ruínas, da vegetação e do céu.";
    window.setTimeout(function () {
  ...
  preload.addEventListener("error", function () {
    resetGame();
    messageOutput.textContent = "A imagem do quebra-cabeça não foi encontrada. Confira assets/img/jogos/forno-da-penha.webp.";
  });
