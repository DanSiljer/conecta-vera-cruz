(() => {
  ...
  const elements = {
    name: document.querySelector("#mat-mural-nome"),
    message: document.querySelector("#mat-mural-mensagem"),
    counter: document.querySelector("#mat-mural-contador"),
    warning: document.querySelector("#mat-mural-aviso"),
    submit: document.querySelector("#mat-mural-enviar"),
    status: document.querySelector("#mat-mural-status"),
    total: document.querySelector("#mat-mural-total"),
    list: document.querySelector("#mat-mural-lista")
  };

  if (Object.values(elements).some((element) => !element)) return;

  const LOCATION = "Matarandiba";
  const COLLECTION = "memorias";
  const MIN_LENGTH = 10;
  const MAX_LENGTH = 500;
  const WAIT_MS = 60_000;
  const STORAGE_KEY = "cv-matarandiba-last-post";
  let database;
  ...
  function renderMemories(snapshot) {
    ...
    if (!memories.length) {
      elements.list.innerHTML = `
        <div class="mat-memory-empty">
          Ainda não há memórias publicadas sobre Matarandiba. A primeira pode ser a sua.
        </div>`;
      return;
    }
    ...
    elements.list.innerHTML = memories.map((memory) => {
      ...
      return `
        <article class="mat-memory-item">
          <div class="mat-memory-item__top">
            <strong>${name}</strong>
  ...
  async function submitMemory() {
    ...
    if (elapsed < WAIT_MS) {
      const seconds = Math.ceil((WAIT_MS - elapsed) / 1000);
      setStatus(`Aguarde ${seconds} segundos antes de publicar novamente.`, "warning");
      return;
    }
    ...
    try {
      ...
      if (error?.code === "permission-denied") {
        ...
        setStatus("Não foi possível publicar agora. Tente novamente em alguns instantes.", "error");
      }
    } finally {
  ...
  try {
    ...
    elements.list.innerHTML = '<div class="mat-memory-empty">Conectando ao mural do Conecta Vera Cruz...</div>';
    ...
    database
      ...
      .onSnapshot(renderMemories, (error) => {
        ...
        elements.list.innerHTML = '<div class="mat-memory-empty">Não foi possível carregar o mural neste momento.</div>';
      });
  } catch (error) {
    console.error("Erro ao iniciar Firebase:", error);
    elements.list.innerHTML = '<div class="mat-memory-empty">O mural não pôde ser iniciado.</div>';
  }
