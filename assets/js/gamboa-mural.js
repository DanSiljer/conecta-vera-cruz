(() => {
  ...
  const nomeInput = document.getElementById("vc-gamboa-nome");
  const mensagemInput = document.getElementById("vc-gamboa-mensagem");
  const enviarButton = document.getElementById("vc-gamboa-enviar");
  const lista = document.getElementById("vc-gamboa-lista");
  const contador = document.getElementById("vc-gamboa-contador");
  const total = document.getElementById("vc-gamboa-total");
  const aviso = document.getElementById("vc-gamboa-aviso");
  const status = document.getElementById("vc-gamboa-status");

  if (!nomeInput || !mensagemInput || !enviarButton || !lista || !contador || !total || !aviso || !status) return;

  const LIKES_KEY = "conecta-vera-cruz-curtidas-gamboa-v1";
  const LAST_SEND_KEY = "conecta-vera-cruz-ultimo-envio-gamboa-v1";
  let db = null;
  let ready = false;
  let posts = [];
  let liked = loadLiked();
  ...
  function emptyState(message = "Compartilhe a primeira história ou curiosidade sobre a Gamboa.") {
  ...
  function createPost(post) {
    ...
    location.textContent = "📍 Gamboa · Conecta Vera Cruz";
