(function () {
  ...
  const board = document.getElementById("marGrandePuzzleBoard");
  const tray = document.getElementById("marGrandePuzzleTray");
  if (!board || !tray) return;

  const difficulty = document.getElementById("marGrandePuzzleDifficulty");
  const startButton = document.getElementById("marGrandePuzzleStart");
  const showButton = document.getElementById("marGrandePuzzleShow");
  const resetButton = document.getElementById("marGrandePuzzleReset");
  const movesOutput = document.getElementById("marGrandePuzzleMoves");
  const timeOutput = document.getElementById("marGrandePuzzleTime");
  const placedOutput = document.getElementById("marGrandePuzzlePlaced");
  const totalOutput = document.getElementById("marGrandePuzzleTotal");
  ...
  function createPieceSvg(index, ghost) {
    ...
    const clipId = "marGrandePieceClip-" + state.size + "-" + index + "-" + Math.random().toString(36).slice(2, 8);
  ...
  function saveProgress() {
    ...
    results.unshift({
      ...
      activity: "Quebra-cabeça da igreja e da praça de Mar Grande · " + difficultyName,
  ...
  function finishGame() {
    ...
    celebrateCompletion();
    messageOutput.innerHTML = "<strong>Quebra-cabeça concluído!</strong> Você montou a vista aérea de Mar Grande em " + state.moves + " movimentos e " + formatTime(state.seconds) + ". O resultado foi salvo em Meu progresso.";
  ...
  function showHint() {
    ...
    messageOutput.textContent = "Dica rápida: memorize a posição da igreja, da praça, das ruas e dos telhados.";
    window.setTimeout(function () {
